...
| **M3** Module | Clock, network, filesystem, randomness and environment arrive as parameters (Feathers, seams). | Run the unit twice in one process against two clocks, sockets, paths, seeds or environments: if the only lever is patching a module, a global or a private field, the seam is missing. |
| **M4** Module | A new case is a new unit (Wadler, expression problem; Meyer, open–closed). | Duplicate an existing case under a new key and diff: every changed line lands in the new unit or in an extension point that already existed — a sum type with its matches, a table, a registry, a schema. A chain of unrelated predicates hosts no cases. |
...
| **F2** Function | Each body stays inside a complexity budget. | Checker in CI: over McCabe's 10 fails. A flat dispatch on one value is exempt, flat meaning each arm is a single call or return. With no checker in the build, every body is unmeasured, not passing. |
...
| **F4** Function | Branches that can both match state which wins. | Satisfy two branch conditions with one input, then permute the branches: a result that changes was decided by position, not by its condition. Branches no input satisfies together are exempt, and so are guards whose rejections differ in nothing but the message. |
...
the rest; report it there, and report the others separately.
...
- **L6** mutable state a body reaches without receiving it — a module or static binding, a cell held by a module-level closure, a singleton behind an accessor, the environment — written anywhere but its declaration
